...
export class ExportService {
  ...
  async generatePDF(userId: string, res: Response) {
    ...
    doc.moveDown();
    doc.fontSize(12).text(`Total Monthly Cost: ${user.currency} ${analytics.totalMonthlyCost.toFixed(2)}`);
    doc.text(`Total Yearly Cost: ${user.currency} ${analytics.totalYearlyCost.toFixed(2)}`);
    ...
    if (analytics.categoryBreakdown.length > 0) {
      ...
      analytics.categoryBreakdown.forEach((cat) => {
        doc.fontSize(12).text(`${cat.category}: ${user.currency} ${cat.total.toFixed(2)} (${cat.count} subscriptions)`);
      });
      doc.moveDown(2);
    }
    ...
    subscriptions.forEach((sub, index) => {
      ...
      doc.text(` - ${user.currency} ${Number(sub.price).toFixed(2)}/${sub.billingPeriod.toLowerCase()}`);
      doc.fillColor('black'); // Reset to black
      doc.text(`Next Charge: ${new Date(sub.nextChargeDate).toLocaleDateString()}`);
  ...
  async exportSubscriptionsCsv(userId: string): Promise<string> {
    ...
    const headers = 'Name,Price,Billing Period,Category,Next Charge Date,Active,Notes';
