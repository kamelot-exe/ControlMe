...
import { findServiceIcon } from "@/lib/services/icon-registry";
...
function getAccent(logoHint?: string | null) {
  ...
  const value = (hash % 0xffffff).toString(16).padStart(6, "0");
  return `#${value}`;
}

export function ServiceBadge({ name, logoHint, serviceGroup, className }: ServiceBadgeProps) {
  const initials = getInitials(name);
  const icon = findServiceIcon({ name, logoHint, serviceGroup });
  const accent = icon?.accent ?? getAccent(logoHint ?? name);
